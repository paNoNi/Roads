...
model = dict(
    ...
    decode_head=dict(
        ...
        num_classes=6,  # Number of segmentation class. Usually 19 for cityscapes, 21 for VOC, 150 for ADE20k.
        ...
        loss_decode=dict(  # Config of loss function for the decode_head.
            ...
            loss_weight=1.0),
        sampler=dict(type='OHEMPixelSampler', thresh=0.0, min_kept=100000)
    ),  # Loss weight of decode head.
    auxiliary_head=dict(
    ...
    test_cfg=dict(
        mode='whole'))  # The test mode, options are 'whole' and 'sliding'. 'whole': whole image fully-convolutional test. 'sliding': sliding crop window on the image.
...
train_pipeline = [  # Training pipeline.
    ...
    # Augmentation pipeline that distort current image with several photo metric methods.
    dict(
...
classes = ['column', 'fence', 'road', 'curb', 'hatch', 'traffic_lights']
data = dict(
...
optimizer = dict(
    ...
    type='SGD',
    # Type of optimizers, refer to https://github.com/open-mmlab/mmcv/blob/master/mmcv/runner/optimizer/default_constructor.py#L13 for more details
...
lr_config = dict(
    policy='poly',
    # The policy of scheduler, also support Step, CosineAnnealing, Cyclic, etc. Refer to details of supported LrUpdater from https://github.com/open-mmlab/mmcv/blob/master/mmcv/runner/hooks/lr_updater.py#L9.
...
checkpoint_config = dict(
    ...
    interval=1000,
    meta = dict(
        PALLETTE=list(range(1, 7)),
        CLASSES=classes,
    ))  # The save interval.
evaluation = dict(
    # The config to build the evaluation hook. Please refer to mmseg/core/evaluation/eval_hook.py for details.
    interval=100,  # The interval of evaluation.
